 ...
 */
export const KnowledgeGraphVisualizer = ({ knowledgeGraph, mcpClient, onNodeSelect, className = '' }) => {
    // Refs
    ...
    const edgesMeshRef = useRef(new Map());
    const labelsMeshRef = useRef(new Map());
    // Interaction
    ...
    // Create Poincaré disk boundary
    const createPoincareDisc = (scene) => {
    ...
    const createHyperbolicGrid = (scene) => {
    ...
    const hyperbolicMath = {
        ...
        toPoincare: (h) => {
            ...
            return new THREE.Vector3(factor * h.x / norm, 0, factor * h.y / norm);
        },
        ...
        mobiusTransform: (z, center) => {
            const a = center.x;
            const b = center.z;
            const norm = a * a + b * b;
            if (norm >= 1)
                return z; // Invalid center
            const numeratorX = z.x - a;
            const numeratorZ = z.z - b;
            const denominator = 1 - a * z.x - b * z.z;
            return new THREE.Vector3(numeratorX / denominator, 0, numeratorZ / denominator);
        },
        // Geodesic interpolation
        geodesic: (start, end, t) => {
            const d = this.distance(start, end);
            if (d === 0)
                return start;
            const lambda = Math.sinh(t * d) / Math.sinh(d);
            return new THREE.Vector3(start.x + lambda * (end.x - start.x), 0, start.z + lambda * (end.z - start.z));
        }
    };
    // Update visualization when knowledge graph changes
    useEffect(() => {
        if (!knowledgeGraph || !isInitialized || !sceneRef.current)
            return;
        updateVisualization();
    }, [knowledgeGraph, isInitialized, settings]);
    // Update visualization
    const updateVisualization = () => {
        if (!knowledgeGraph || !sceneRef.current)
            return;
        clearVisualization();
    ...
    const calculateHyperbolicPositions = (nodes) => {
    ...
    const createNodeMesh = (node, position) => {
        const geometry = new THREE.SphereGeometry(settings.nodeSize * getNodeSizeMultiplier(node), 16, 16);
    ...
    const createEdgeLine = (edge, start, end) => {
        const points = [];
        const segments = 20;
    ...
    const getNodeSizeMultiplier = (node) => {
    ...
    return (_jsxs("div", { className: `relative h-full bg-gray-950 ${className}`, children: [_jsxs("div", { className: "absolute top-4 left-4 right-4 z-10 flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("h3", { className: "text-lg font-semibold text-white", children: "\uD83D\uDD78\uFE0F Knowledge Graph" }), knowledgeGraph && (_jsxs("span", { className: "text-sm text-gray-400", children: [knowledgeGraph.nodes.length, " nodes, ", knowledgeGraph.edges.length, " edges"] }))] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsxs("div", { className: "relative", children: [_jsx(Search, { className: "absolute left-2 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" }), _jsx("input", { type: "text", placeholder: "Search nodes...", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), className: "pl-8 pr-3 py-1 bg-gray-800 border border-gray-600 rounded text-sm text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" })] }), _jsxs("select", { value: settings.layout, onChange: (e) => setSettings(prev => ({ ...prev, layout: e.target.value })), className: "bg-gray-800 border border-gray-600 rounded px-2 py-1 text-sm text-white", children: [_jsx("option", { value: "hyperbolic", children: "Hyperbolic" }), _jsx("option", { value: "force", children: "Force" }), _jsx("option", { value: "hierarchical", children: "Hierarchical" }), _jsx("option", { value: "circular", children: "Circular" })] }), _jsx("button", { onClick: () => setIsAnimating(!isAnimating), className: "p-2 bg-gray-800 hover:bg-gray-700 rounded transition-colors", children: isAnimating ? (_jsx(Pause, { className: "h-4 w-4 text-white" })) : (_jsx(Play, { className: "h-4 w-4 text-white" })) }), _jsx("button", { onClick: () => {
                                    if (cameraRef.current) {
                                        cameraRef.current.position.set(0, 0, 5);
                                        cameraRef.current.lookAt(0, 0, 0);
                                    }
                                }, className: "p-2 bg-gray-800 hover:bg-gray-700 rounded transition-colors", children: _jsx(RotateCcw, { className: "h-4 w-4 text-white" }) })] })] }), _jsx("div", { ref: containerRef, className: "w-full h-full", onMouseMove: handleMouseMove, onClick: handleClick }), !isInitialized && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-gray-950", children: _jsx(motion.div, { animate: { rotate: 360 }, transition: { duration: 2, repeat: Infinity, ease: "linear" }, className: "w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full" }) })), selectedNode && (_jsxs(motion.div, { initial: { opacity: 0, x: 300 }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0, x: 300 }, className: "absolute top-16 right-4 w-80 bg-gray-800 border border-gray-600 rounded-lg p-4 z-20", children: [_jsx("h4", { className: "text-lg font-semibold text-white mb-2", children: "Node Details" }), _jsx("button", { onClick: () => setSelectedNode(null), className: "absolute top-2 right-2 text-gray-400 hover:text-white", children: "\u00D7" })] })), _jsxs("div", { className: "absolute bottom-4 left-4 text-xs text-gray-400", children: ["Layout: ", settings.layout, " | Render: ", settings.renderMode, " |", hoveredNode && ` Hover: ${hoveredNode}`] })] }));
};
//# sourceMappingURL=KnowledgeGraphVisualizer.js.map
...
// Create the store
export const useIDEStore = create()(subscribeWithSelector(immer((set, get) => ({
    // Initial state
    currentProject: null,
    activeFile: null,
    knowledgeGraph: null,
    ...
    resizePanel: (panel, dimension, value) => set((state) => {
        const panelState = state.layout[panel];
...
useIDEStore.subscribe((state) => state, (state) => {
...
export const useRunningAgents = () => useIDEStore((state) => state.activeAgents.filter(agent => agent.status === 'running'));
export const useSelectedNodes = () => useIDEStore((state) => state.layout.knowledgeGraph.selectedNodes);
//# sourceMappingURL=ideStore.js.map
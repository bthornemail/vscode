...
export function createHyperbolicVector(coords) {
...
export function mobiusAddition(u, v) {
    ...
    const numerator = u.value.map((u_i, i) => gamma_u * u_i + gamma_v * v.value[i] +
...
export function hyperbolicToEuclidean(hypCoords, radius = 1) {
    ...
    return new Vector3(hypCoords.value[0] * scale, hypCoords.value[1] * scale, hypCoords.value[2] || 0 // Handle 2D coordinates
    );
}
...
const HyperbolicTransformComponent = {
    ...
    tick(time) {
        if (this.animating) {
            ...
            this.currentHyperbolic = this.interpolateHyperbolic(this.currentHyperbolic, this.targetHyperbolic, t);
            this.updatePosition();
            if (progress >= 1) {
                this.animating = false;
                this.currentHyperbolic = this.targetHyperbolic;
    ...
    getHyperbolicPosition() {
        return this.currentHyperbolic;
    },
    setHyperbolicPosition(coords, animate = false) {
...
if (typeof window !== 'undefined' && window.AFRAME) {
    window.AFRAME.registerComponent('hyperbolic-transform', HyperbolicTransformComponent);
}
export { HyperbolicTransformComponent };
//# sourceMappingURL=hyperbolic-transform.js.map
...
async function runAutonomousCoordinationExample() {
    ...
    const server = new src_1.MCPServer({
        port: 3000,
        host: 'localhost',
        transport: {
            type: 'websocket',
    ...
    for (const task of tasks) {
        ...
        if (task.type === 'data-processing')
            targetGroupId = groupIds[0];
        else if (task.type === 'coordination')
            targetGroupId = groupIds[1];
        else if (task.type === 'monitoring')
            targetGroupId = groupIds[2];
        if (targetGroupId) {
            const taskMessage = {
                taskId: task.id,
                type: task.type,
...
//# sourceMappingURL=autonomous-coordination.js.map